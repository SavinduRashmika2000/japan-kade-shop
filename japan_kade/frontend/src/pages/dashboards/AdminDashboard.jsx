          ...
          {activeTab === 'inventory' && (
            ...
            >
              ...
              <motion.div variants={itemVariants} className="bg-white border border-slate-100 rounded-[2rem] shadow-sm overflow-hidden mb-10">
                ...
                <div className="block md:hidden p-4 space-y-6 bg-slate-50/30">
                  {groupedPaginatedStock.map(([categoryName, items]) => (
                    <div key={categoryName} className="space-y-3">
                      ...
                      <div className="flex items-center gap-2 px-2">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{categoryName}</span>
                        <span className="text-[9px] font-bold text-slate-400 bg-white px-2 py-0.5 rounded-full border border-slate-100">{items.length}</span>
                      </div>
                      ...
                      {items.map((item, index) => {
                        ...
                        return (
                          <div key={item.id || `stock-mobile-${index}`} className={`p-4 bg-white border border-slate-100 rounded-2xl shadow-sm hover:shadow-md transition-all space-y-3 ${
                            isExpanded ? 'bg-slate-50/20 ring-1 ring-slate-100' : ''
                          }`}>
                            <div className="flex justify-between items-start gap-2">
                              <div className="min-w-0 flex-1">
                                <h4 className="font-black text-slate-900 text-sm tracking-tight break-words">{item.name}</h4>
                                <div className="flex items-center gap-2 mt-1">
                                  <span className="text-[9px] font-bold text-slate-400 bg-slate-50 px-1.5 py-0.5 rounded border border-slate-100 uppercase tracking-tighter shrink-0">{item.partNumber}</span>
                                  {item.hsCode && <span className="text-[8px] font-black text-slate-300 uppercase tracking-widest truncate">HS: {item.hsCode}</span>}
                                </div>
                              </div>
                              <button 
                                onClick={() => toggleRowExpansion(item.id)}
                                className={`w-7 h-7 rounded-lg flex items-center justify-center transition-all shrink-0 ${isExpanded ? 'bg-slate-900 text-white rotate-180' : 'bg-slate-100 text-slate-400'}`}
                              >
                            ...
                            <div className="pt-2 border-t border-slate-100 text-[11px] space-y-1.5">
                              {item.location && (
                                <div className="flex items-start gap-1.5">
                                  ...
                                  <span className="font-semibold text-slate-700 leading-tight">Location: {item.location}</span>
                                </div>
                              )}
                              {item.remarks && (
                                <div className="flex items-start gap-1.5">
                                  <span className="text-[10px] shrink-0">📝</span>
                                  <span className="text-slate-500 italic leading-tight">{item.remarks}</span>
                                </div>
                              )}
                              {!item.location && !item.remarks && (
                                <p className="text-[10px] text-slate-300 italic">No location or remarks listed</p>
                              )}
                            </div>

                            <div className="pt-2 border-t border-slate-100 flex justify-between items-center gap-2">
                              <div className="space-y-0.5">
                                <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider block">Stock Level</span>
                                <div className="flex items-center gap-2">
                                  <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest ${isOutOfStock ? 'bg-red-50 text-red-600' : isLowStock ? 'bg-amber-50 text-amber-600' : 'bg-emerald-50 text-emerald-600'}`}>
                                    {isOutOfStock ? 'Empty' : isLowStock ? 'Low Stock' : 'Good'}
                                  </span>
                                  <span className="text-xs font-black text-slate-900">{item.quantity} units</span>
                                </div>
                              ...
                              <div className="text-right">
                                <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider block">Unit Price</span>
                                <span className="text-sm font-black text-slate-900">Rs. {parseFloat(item.unitPrice || 0).toLocaleString()}</span>
                              </div>
                            </div>
                            ...
                            <div className="pt-2 border-t border-slate-50 flex justify-between items-center gap-2">
                              ...
                              <div className="flex items-center gap-2">
                                <button onClick={() => {
                                  setQuickAddData({ itemId: item.id, quantity: '', hsCode: '', currencyType: '', unitCostForeign: '', exchangeRate: '', freightCost: '', shippingCost: '', bankCharges: '', clearanceFees: '', dutyFees: '', additionalExpenses: '', landedCost: '', unitPrice: item.unitPrice || '', sellingPrice: item.unitPrice || '', supplierId: item.supplier?.id || '' });
                                  setShowQuickAddModal(true);
                                }} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-emerald-50 text-emerald-600 hover:bg-emerald-600 hover:text-white transition-all text-[9px] font-black uppercase tracking-widest border border-emerald-100">
                                  <Plus className="w-3.5 h-3.5" /> Stock In
                                </button>
                                <button onClick={() => {
                                  setQuickReduceData({ itemId: item.id, quantity: '', reason: '' });
                                  setShowQuickReduceModal(true);
                                }} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-rose-50 text-rose-600 hover:bg-rose-600 hover:text-white transition-all text-[9px] font-black uppercase tracking-widest border border-rose-100">
                                  <Minus className="w-3.5 h-3.5" /> Stock Out
                                </button>
                                <button onClick={() => openEditStock(item)} className="p-1.5 rounded-lg bg-slate-50 hover:bg-blue-600 text-slate-400 hover:text-white transition-all border border-slate-100 hover:border-blue-600">
                                  <Pencil className="w-3.5 h-3.5" />
                                </button>
                              </div>
                            </div>
                            ...
                            {isExpanded && (
                              <div className="pt-3 border-t border-slate-100 overflow-hidden">
                                <div className="bg-slate-50/50 rounded-xl border border-slate-100 overflow-hidden">
                                  <div className="px-3 py-2 bg-slate-100/50 border-b border-slate-100 flex items-center gap-2">
                                    <Layers className="w-3 h-3 text-indigo-500" />
                                    <span className="text-[8px] font-black uppercase tracking-widest text-slate-500">Batch Inventory Ledger (FIFO)</span>
                                  </div>
                                  
                                  <div className="p-2 space-y-2">
                                    {(rowBatches[item.id] || []).map((batch, bi) => (
                                      <div key={batch.id || bi} className="bg-white border border-slate-100 rounded-lg p-2.5 space-y-1.5 text-[10px]">
                                        <div className="flex justify-between items-start gap-1">
                                          <div>
                                            <p className="font-black text-slate-600">{new Date(batch.createdAt).toLocaleDateString()}</p>
                                            <p className="text-[8px] font-bold text-slate-400">{new Date(batch.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                                          </div>
                                          <span className={`font-black px-1.5 py-0.5 rounded text-[8px] ${batch.currentQuantity === 0 ? 'bg-slate-100 text-slate-400' : 'bg-blue-50 text-blue-600'}`}>
                                            Qty: {batch.currentQuantity} / {batch.initialQuantity}
                                          </span>
                                        </div>
                                        <div className="flex justify-between items-center gap-2 border-t border-slate-50 pt-1.5">
                                          <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">
                                            Supplier: <span className="text-slate-600 normal-case font-bold">{batch.supplier?.companyName || 'N/A'}</span>
                                          </p>
                                          <p className="font-black text-slate-900">Rs. {(batch.sellingPrice || batch.unitPrice || 0).toLocaleString()}</p>
                                        </div>
                                        <div className="flex justify-end pt-1">
                                          <button
                                            type="button"
                                            onClick={() => setSelectedBatchDetail(batch)}
                                            className="px-2 py-0.5 rounded bg-slate-100 hover:bg-slate-900 hover:text-white text-[8px] font-black uppercase tracking-wider transition-all border border-slate-200"
                                          >
                                            Details
                                          </button>
                                        </div>
                                    ...
                                    {(!rowBatches[item.id] || rowBatches[item.id].length === 0) && (
                                      <p className="py-3 text-center text-[9px] font-bold text-slate-400 italic">No batch history found.</p>
                                    )}
                                  </div>
                                </div>
                              </div>
                  ...
                  {paginatedStock.length === 0 && (
                    <p className="py-10 text-center text-slate-400 font-bold italic bg-white rounded-2xl border border-slate-100">No inventory records found.</p>
                  )}
                </div>
                        ...
                        <tbody className="divide-y divide-slate-50">
                          {paginatedTransactions.map((tx) => {
                            ...
                            <React.Fragment key={tx.id}>
                              ...
                              {tx.isGroup && (
                                <tr>
                                  <td colSpan={logTab === "stock_out" ? 7 : 6} className="px-4 pb-4">
